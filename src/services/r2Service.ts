...
export class R2Service {
      ...
      let albums = await Promise.all(
        (data.CommonPrefixes || []).map(async (prefix) => {
          ...
          try {
            ...
            const mp3File = (albumFiles.Contents || []).find(file => 
            ...
            if (mp3File) {
              try {
                ...
                logger.error(`Error getting metadata for ${mp3File.Key}`, { error: metaError });
              }
            }
          }
  ...
  async getAlbum(name: string, limit?: number, offset?: number): Promise<Album> {
    try {
      ...
      try {
        ...
        // album.json doesn't exist or couldn't be parsed, try metadata approach
        // Find the first MP3 file and get its metadata
        const mp3File = (data.Contents || []).find(file => 
          file.Key!.endsWith('.mp3')
        );
  ...
  async listAllSongs(limit?: number, offset?: number, shuffle?: boolean): Promise<MusicCollection> {
    try {
      ...
      const albumJsons: Record<string, boolean> = {};
      ...
      await Promise.all(
        Object.keys(albumJsons).map(async (albumName) => {
          try {
            ...
            logger.error(`Error getting album.json for ${albumName}`, { error });
          }
        })
      );
      ...
      let allSongs = await Promise.all(
   ...
   */
  async listArtists(): Promise<{ artists: Array<{ name: string, albums: number }> }> {
    try {
        ...
        (data.CommonPrefixes || []).map(async (prefix) => {
          ...
          try {
            ...
            try {
              ...
              // album.json doesn't exist or couldn't be parsed, try metadata approach
              logger.info(`No album.json found for ${albumName}, trying metadata approach`);
            }
            ...
            logger.error(`Error processing album ${albumName} for artists`, { error });
          }
        })
      );
      ...
      logger.error('Error listing artists from R2', { error });
      throw error;
    }
  }
   ...
   */
  async getAlbumsByArtist(artistName: string, limit?: number, offset?: number): Promise<{ artist: string, albums: Album[] }> {
    try {
      // First, get all albums
      const data = await s3.listObjectsV2({
        Bucket: this.bucketName,
      ...
      await Promise.all(
        (data.CommonPrefixes || []).map(async (prefix) => {
          ...
          try {
            ...
            logger.error(`Error processing album ${albumName} for artist ${artistName}`, { error });
          }
        })
      );
      ...
      logger.error(`Error getting albums for artist ${artistName} from R2`, { error });
      throw error;
    }
  }
   ...
   */
  async getSongsByArtist(artistName: string, limit?: number, offset?: number, shuffle?: boolean): Promise<{ artist: string, totalSongs: number, songs: Song[] }> {
    try {
      ...
      await Promise.all(
        albums.map(async (album) => {
          try {
            ...
            logger.error(`Error getting songs from album ${album.name} for artist ${artistName}`, { error });
          }
        })
      );
      ...
      logger.error(`Error getting songs for artist ${artistName} from R2`, { error });
      throw error;
    }
  }
